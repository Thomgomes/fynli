    ...
    cell: ({ row }) => {
        ...
        return (
            <div className="flex items-center gap-3">
                {IconComponent && <div className="p-2 bg-muted rounded-md"><IconComponent className="h-4 w-4 text-muted-foreground" /></div>}
                <div>
                    <div className="font-medium">{row.getValue("description")}</div>
                    <div className="text-sm text-muted-foreground">{category?.name || 'Sem Categoria'}</div>
                </div>
            </div>
        )
    }
